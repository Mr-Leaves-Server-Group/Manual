...
## 2.后台简介
- **🪵【必填】服务器类型**  
<small>见下方的列表，切换不同服务器类型，将影响安装脚本的识别逻辑</small>  
<small>**⚠️注意：切换后，请手动删除所有文件，并在【设置】分页点击 “重装服务器”**</small>  

| 服务器类型 | 适用场景 | 核心特点 | 插件/模组支持 |
| :--- | :--- | :--- | :--- |
| **Vanilla (原版)** | 极简生存、好友开荒 | 官方纯净体验，最稳定，通常版本最新 | 不支持 |
| **Paper (插件)** | 商业服、多人社区 | 插件门户服，性能占用最低，防作弊强 | 仅插件  |
| **Forge (模组)** | 大型模组生存 | 历史悠久，模组生态最丰富，兼容性强 | 大型模组 |
| **NeoForge (模组)** | 1.20.1+ 现代模组服 | Forge 的现代化分支，性能与架构更优 | 大型模组 |
| **Fabric (模组)** | 生电、轻量化生存 | 启动极快，占用内存小，生电玩家首选 | 轻量模组 |
| **CurseForge (整合包)** | 国际大作 (如 RLCraft) | 自动从CF下载整合包，无需手动上传 | 预制整合包 |
| **GuoChan (整合包)** | 国内热门整合包 | 流行的全中文包，一键安装，新手友好 | 预制整合包 |
...
- **🔴【高级】手动游戏版本**  
<small> 填写不包含在常用列表中的版本（如 1.20.6）留空不生效，修改后请务切换右上角 Java镜像版本</small>

- **🧩【模组】具体版本号**   
<small>必须和客户端一样，填写示例：Paper `100`|Fabric `0.16.13`|Forge `47.4.2`|NeoForge `20.4.194`</small>

- **🧩【模组】Fabric版本（API模组）**  
<small>自动安装 API 模组，Fabric模式专用。填写格式示例：`0.92.0+1.20.1`</small>

- **📦【整合包】CF模组包-Project ID**  
<small>填写 CF模组包 项目ID（位于页面右侧）例如：RL-Craft 为 `285109` <a href="#36curseforge">点击查看获取方法</a></small>

- **📦【整合包】CF模组包-File ID**  
<small>填写 CF模组包 文件ID（位于文件URL最后）例如：RL-Craft 最新服务包为 `4612990` <a href="#36curseforge">点击查看获取方法</a></small>

- **📦【整合包】国产模组包-ModPack**  
<small>选择预设好的国产整合包服务端，如《愚者》或《乌托邦》<a href="#37guochan">点击查看模组包列表</a></small>

![后台界面](\assets\minecraft\minecraft-egg-backend.png)  

- **🎮【游戏】服务器MOTD**  
<small>服务器在联机列表显示的简短介绍，部分版本不支持中文，推荐使用英文</small>

- **🎮【游戏】正版验证**  
<small>开关正版验证模式。关闭后离线玩家使用PCL2客户端可进入，但所有玩家皮肤将会失效</small>

- **🎮【游戏】游戏难度**  
<small>设置世界难度：peaceful (和平), easy (简单), normal (普通), hard (困难)</small>

- **🎮【游戏】游戏模式**  
<small>设置默认模式：survival (生存), creative (创造), adventure (冒险), spectator (旁观)</small>

- **🎮【游戏】最大玩家数量**  
<small>设置服务器最大同时在线人数， 不推荐设置过高</small>

- **🎮【游戏】地图种子**  
<small>注：留空则使用随机种子，推荐使用<a href="https://www.chunkbase.com/apps/seed-map">Chunkbase Seed Map</a>查看种子地形</small>

- **🎮【游戏】允许飞行**  
<small>是否允许玩家在生存模式下飞行。模组服建议开启此项</small>

## 3.开服教程
### 3.1.Java版本选择
**由于MC各类分支较多，请参考游戏版本选择后台的Java版本**

| Minecraft版本号                  | Java(JDK) 推荐版本 |
| ------------------------------- | ---------------- |
| 1.7 ~ 1.12.2                    | 8                |
| 1.16 ~ 1.16.5                   | 8/11             |
| 1.17 ~ 1.20.5                   | 17               |
...
### 3.6.CurseForge（整合包）
#### 3.6.1.服务器端自动安装  
**✅菜菜云MLSG支持CF模组包的服务器端自动安装**  
**ℹ️按照以下教程操作，即可3分钟安装对应服务器端**  
...
点击打开➡ **[菜菜云MLSG - 国产包列表](https://manual.mr-leaves.com/tools/guochan_metadata.html)**，右键复制链接 **分享给好友一块选择** 哦！
...
> 1. 下载PCL2客户端，点击 **[下载链接](/assets/minecraft/PCL2-2.13.1.1-20260816.exe)** 下载
> 2. 点击运行PCL2客户端，点击左下角 **版本选择**
> 3. 在版本选择界面，点击左侧的 **安装模组包**
> 4. 选择模组包 zip 文件，点击 **打开**
> 5. 等待安装完成后，选择右侧安装好的模组包
> 6. 返回主界面，点击 **启动游戏**

> 0.感谢PCL2的原作开发者，点击 **[支持作者](https://afdian.com/a/LTCat)** 为作者发电
...
- 由于模组服对服务器/客户端版本对应要求非常严格，推荐新玩家从CF打包好的模组包开始。**[CurseForge官网 - 模组包列表](https://curseforge.com/minecraft/modpacks)**  
- 更多其他第三方模组包（如MCmod，B站，笔记），需要玩家自己**提供服务器端模组包**，以及自己**分发客户端模组包**，且需保证所有玩家客户端版本和模组统一。  
<br>

## 5.地图载入/生成速度

- **地图载入（已经探索的区域）**主要和内存大小有关，如果内存不够大，已经生成的地图则会载入很慢，一般8G足以，16G-24G最优，**大内存对 倍速/飞行 会有显著提升**。 
- **游戏生成（未探索过的区域）**和CPU有关系，绝大部分例如Paper和Spigot后端都支持多核心的地图渲染，多核心会显著提升地图生成速度。但对于多数模组服务器通常没有帮助。
- **模组服务器（单核心地图生成）**都会很慢，可以使用对应版本的 **[Chunky模组/插件](https://www.mcmod.cn/class/6239.html)** 来预生成地图，以提升玩家体验。
<br>
...
- `keep-inventory`：死亡是否掉落物品，默认false，改成true后玩家死亡会保留物品
- `motd`：服务器MOTD，在联机列表显示的服务器简介，部分版本不支持中文，建议使用英文
